...
// registrado_por en `ingresos` es NOT NULL + FK a usuarios: el uid local de
// Rojas no existe en la base de otra sede, así que para sedes se resuelve un
// usuario válido de esa base (director/admin). null si no hay ninguno.
async function usuarioParaIngreso(db: SupabaseClient, esExterno: boolean, localUserId: string): Promise<string | null> {
  if (!esExterno) return localUserId
  const { data } = await db.from('usuarios').select('id').in('rol', ['jose', 'director', 'admin']).limit(1).maybeSingle()
  return data?.id ?? null
}
...
// Federado: opera contra la base de la sede donde vive la proforma. Opcional-
// mente asocia anticipos/ingresos que el cliente ya pagó (descuenta saldo del
// anticipo y liga los ingresos a la proforma de precompra), sin dejar de
// registrar los $500 en bóveda + el % en contabilidad.
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  // Resuelve la base de la proforma (local o la de otra sede).
  ...
  const { db: supabase, proforma: pf, externo } = resuelta
  const esExterno = !!externo
  ...
  // 1) $500 fijos → BÓVEDA (creado_por sin FK: se guarda el uid de quien registra)
  ...
  // (a) Ingresos YA registrados → se ligan a la proforma de precompra
  //     (precompra_proforma_id; NO proforma_id, que apunta a ventas de piso).
  if (ingresoIds.length) {
    const { data: ings } = await supabase.from('ingresos')
      .update({ precompra_proforma_id: id })
      .in('id', ingresoIds)
      .select('id, monto')
  ...
  //     de precompra, descontando su saldo (topado a lo que quede de la cuota 1).
  if (anticipoIds.length && pf.cliente_id) {
    const registradoPor = await usuarioParaIngreso(supabase, esExterno, user.id)
    if (registradoPor) {
