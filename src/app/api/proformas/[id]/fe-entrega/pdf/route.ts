...
import { FeEntregaPDF, FeEntregaData } from '@/lib/fe-entrega-pdf'
...
export async function GET(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const buffer = await renderToBuffer(React.createElement(FeEntregaPDF, { data }) as any)
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  return new NextResponse(buffer as any, {
    headers: { 'Content-Type': 'application/pdf', 'Content-Disposition': `inline; filename="fe-entrega-${pro.numero}.pdf"` },
  })
}
