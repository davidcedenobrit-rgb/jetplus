...
const CLICS_VALIDOS = ['concesionario_virtual_click', 'ac500_concesionario_virtual_click']
...
export async function GET() {
  ...
  const [{ data: eventos }, { data: leads }] = await Promise.all([
    supabase.from('eventos_link_ventas').select('evento, marca, modelo, created_at').eq('origen', 'redes').order('created_at', { ascending: false }),
    supabase.from('leads_captacion').select('nombre, telefono, marca, modelo, origen, created_at').neq('origen', 'vendedor_lead').order('created_at', { ascending: false }),
  ])
  ...
  // ── Hoja 1: Mapa de calor (clics por vehículo) ────────────────────
  const heat: Record<string, number> = {}
  ;(eventos ?? []).filter(e => CLICS_VALIDOS.includes(e.evento)).forEach(e => {
    const k = `${e.marca ?? ''} ${e.modelo ?? ''}`.trim() || 'Sin especificar'
    heat[k] = (heat[k] ?? 0) + 1
  })
  const heatArr = Object.entries(heat).sort((a, b) => b[1] - a[1])

  const wsH = wb.addWorksheet('Mapa de calor', { views: [{ state: 'frozen', ySplit: 5 }] })
  wsH.columns = [{ width: 32 }, { width: 14 }]
  encabezado(wsH, 'B', 'MAPA DE CALOR · REDES', `Generado el ${hoy}  ·  ${heatArr.reduce((s, [, n]) => s + n, 0)} clics "Ir a concesionario"`)
  filaEncabezadoTabla(wsH, ['Vehículo', 'Clics'])
  heatArr.forEach(([modelo, clics], idx) => {
    const r = wsH.addRow([modelo, clics])
  ...
  // ── Hoja 2: Clientes enviados por WhatsApp ────────────────────────
  const wsL = wb.addWorksheet('Clientes WhatsApp', { views: [{ state: 'frozen', ySplit: 5 }], pageSetup: { orientation: 'landscape', fitToPage: true, fitToWidth: 1, fitToHeight: 0 } })
  wsL.columns = [{ width: 22 }, { width: 16 }, { width: 24 }, { width: 16 }, { width: 18 }]
  encabezado(wsL, 'E', 'CLIENTES ENVIADOS DESDE REDES', `Generado el ${hoy}  ·  ${(leads ?? []).length} cliente${(leads ?? []).length === 1 ? '' : 's'}`)
  filaEncabezadoTabla(wsL, ['Cliente', 'Teléfono', 'Vehículo', 'Origen', 'Fecha'])
  ;(leads ?? []).forEach((l, idx) => {
    const r = wsL.addRow([l.nombre, l.telefono, `${l.marca ?? ''} ${l.modelo ?? ''}`.trim() || '—', l.origen || 'redes_sociales', fmtFecha(l.created_at)])
    r.eachCell({ includeEmpty: true }, cell => {
      cell.font = { name: 'Calibri', size: 10, color: { argb: NEGRO } }
      cell.alignment = { vertical: 'middle', horizontal: 'left', indent: 1 }
      cell.border = { bottom: { style: 'hair', color: { argb: GRIS_LINEA } } }
  ...
  return new NextResponse(buf as ArrayBuffer, {
    ...
    headers: {
      ...
      'Content-Disposition': `attachment; filename="bitacora-redes-${stamp}.xlsx"`,
