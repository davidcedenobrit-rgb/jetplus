...
export async function GET() {
  ...
  const { data: leads } = await admin
    ...
    .select('nombre, telefono, marca, modelo, origen, created_at')
    .neq('origen', 'vendedor_lead')
  ...
  const data: TablaPDFData = {
    ...
    filas: (leads ?? []).map(l => [
      l.nombre, l.telefono, `${l.marca ?? ''} ${l.modelo ?? ''}`.trim() || '—', l.origen || 'redes_sociales', fmtFecha(l.created_at),
    ]),
  }
  ...
  return new NextResponse(Buffer.from(buffer), {
    status: 200,
    headers: { 'Content-Type': 'application/pdf', 'Content-Disposition': 'attachment; filename="bitacora-redes.pdf"' },
  })
}
